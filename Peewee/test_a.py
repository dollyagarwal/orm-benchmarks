...
def populate(n):
    ...
    print(f"Peewee Testcase A, Warehouse: Rows: {nrows}, Time: {now - start}")
    print(f"Peewee Testcase A, Warehouse: Rows/sec: {nrows / (now - start): 10.2f}")
    ...
    query_res = {'Testcase': testcase, 'Table': 'District', 'Rows': nrows, 'Total time': f"{(now - start):10.2f}",
                 'RowsPerSec': f"{nrows / (now - start): 10.2f}", "ORM": orm}
    result = result.append(query_res, ignore_index=True)
    print(f"Peewee Testcase A, District: Rows: {nrows}, Time: {now - start}")
    print(f"Peewee Testcase A, District: Rows/sec: {nrows / (now - start): 10.2f}")
    ...
    query_res = {'Testcase': testcase, 'Table': 'Customer', 'Rows': nrows, 'Total time': f"{(now - start):10.2f}",
                 'RowsPerSec': f"{nrows / (now - start): 10.2f}", 'ORM': orm}
    result = result.append(query_res, ignore_index=True)
    print(f"Peewee Testcase A, Customer: Rows: {nrows}, Time: {now - start}")
    print(f"Peewee Testcase A, Customer: Rows/sec: {nrows / (now - start): 10.2f}")
    ...
    query_res = {'Testcase': testcase, 'Table': 'Item', 'Rows': nrows, 'Total time': f"{(now - start):10.2f}",
                 'RowsPerSec': f"{nrows / (now - start): 10.2f}", "ORM": orm}
    result = result.append(query_res, ignore_index=True)
    print(f"Peewee Testcase A, Item: Rows: {nrows}, Time: {now - start}")
    print(f"Peewee Testcase A, Item: Rows/sec: {nrows / (now - start): 10.2f}")
    ...
    query_res = {'Testcase': testcase, 'Table': 'Stock', 'Rows': nrows, 'Total time': f"{(now - start):10.2f}",
                 'RowsPerSec': f"{nrows / (now - start): 10.2f}", "ORM": orm}
    result = result.append(query_res, ignore_index=True)
    print(f"Peewee Testcase A, Stock: Rows: {nrows}, Time: {now - start}")
    print(f"Peewee Testcase A, Stock: Rows/sec: {nrows / (now - start): 10.2f}")
    print(result)
    result.to_csv('peewee_outputs/test_a.csv', encoding='utf-8')
...
def main():
    ...
    now = time.time()
    print(f"Test A: total runtime: {(now - start): 10.2f}")
