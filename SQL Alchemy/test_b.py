...
def populate(n):
    ...
    print(f"SQLAlchemy Testcase B, Warehouse: Rows: {nrows}, Time: {(now - start): 10.2f}")
    print(f"SQLAlchemy Testcase B Warehouse: Rows/sec: {nrows / (now - start): 10.2f}")
    ...
    query_res = {'Testcase': testcase, 'Table': 'District', 'Rows': nrows, 'Total time': f"{(now - start):10.2f}",
                 'RowsPerSec': f"{nrows / (now - start): 10.2f}", "ORM": orm}
    result = result.append(query_res, ignore_index=True)
    print(f"SQLAlchemy Testcase B, District: Rows: {nrows}, Time: {(now - start): 10.2f}")
    print(f"SQLAlchemy Testcase B District: Rows/sec: {nrows / (now - start): 10.2f}")
    ...
    query_res = {'Testcase': testcase, 'Table': 'Customer', 'Rows': nrows, 'Total time': f"{(now - start):10.2f}",
                 'RowsPerSec': f"{nrows / (now - start): 10.2f}", "ORM": orm}
    result = result.append(query_res, ignore_index=True)
    print(f"SQLAlchemy Testcase B, Customer: Rows: {nrows}, Time: {(now - start): 10.2f}")
    print(f"SQLAlchemy Testcase B Customer: Rows/sec: {nrows / (now - start): 10.2f}")
    ...
    query_res = {'Testcase': testcase, 'Table': 'Item', 'Rows': nrows, 'Total time': f"{(now - start):10.2f}",
                 'RowsPerSec': f"{nrows / (now - start): 10.2f}", "ORM": orm}
    result = result.append(query_res, ignore_index=True)
    print(f"SQLAlchemy Testcase B, Item: Rows: {nrows}, Time: {(now - start): 10.2f}")
    print(f"SQLAlchemy Testcase B Item: Rows/sec: {nrows / (now - start): 10.2f}")
    ...
    query_res = {'Testcase': testcase, 'Table': 'Stock', 'Rows': nrows, 'Total time': f"{(now - start):10.2f}",
                 'RowsPerSec': f"{nrows / (now - start): 10.2f}", "ORM": orm}
    result = result.append(query_res, ignore_index=True)
    print(f"SQLAlchemy Testcase B, Stock: Rows: {nrows}, Time: {(now - start): 10.2f}")
    print(f"SQLAlchemy Testcase B Stock: Rows/sec: {nrows / (now - start): 10.2f}")
    result.to_csv('sqlalchemy_outputs/test_b.csv', encoding='utf-8')
...
def main():
    ...
    print(f"Test B SQLAlchemy: total runtime: {(now - start): 10.2f}")
